    ...
    <section class="rounded-xl border border-gray-200 bg-white shadow-sm">
        ...
        <div class="divide-y">
            @forelse($events as $event)
                <div class="flex flex-col justify-between gap-2 p-4 sm:flex-row sm:items-center"><div><p class="font-bold">{{ $event->title }}</p><p class="text-xs text-gray-500">{{ $event->event_date->format('F j, Y') }} · {{ $event->start_time }} - {{ $event->end_time }}</p></div><span class="text-xs text-gray-400">{{ $event->creator?->user?->name ?? 'Admin' }}</span></div>
            @empty
