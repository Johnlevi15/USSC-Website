...
@section('content')
	<div class="flex-grow flex items-center justify-center p-4 min-h-[70vh]">
		<div class="bg-white rounded-xl shadow-sm border p-6 w-full max-w-md">
			...
			<form onsubmit="event.preventDefault(); document.getElementById('message').classList.remove('hidden');" class="space-y-4">
				<label class="block text-xs font-bold text-gray-600 uppercase">
					Email Address
					<input required type="email" placeholder="admin@clsu.edu.ph" class="mt-1 w-full px-3 py-2 text-sm border rounded-lg">
				</label>

				<label class="block text-xs font-bold text-gray-600 uppercase">
					Password
					<input required type="password" placeholder="Enter your password" class="mt-1 w-full px-3 py-2 text-sm border rounded-lg">
				</label>

				<button class="w-full py-2.5 bg-red-900 text-white font-bold rounded-lg text-sm">LOG IN</button>
				<p id="message" class="hidden text-center text-xs text-red-700">Admin authentication is ready to connect to the Laravel backend.</p>
			</form>
		</div>
	</div>
@endsection